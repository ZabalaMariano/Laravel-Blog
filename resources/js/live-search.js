...
export default class Search {
  ...
  renderResultsHTML(posts) {
    if (posts.length) {
      ...
      <div class="list-group-item active"><strong>Resultados</strong> (${posts.length > 1 ? `${posts.length} items encontrados` : "1 item encontrado"})</div>
      ${posts
        .map(post => {
        ...
        <span class="text-muted small">por ${post.user.username} el ${postDate.getMonth() + 1}/${postDate.getDate()}/${postDate.getFullYear()}</span>
      </a>`
        })
        .join("")}
    </div>`)
    } else {
      this.resultsArea.innerHTML = `<p class="alert alert-danger text-center shadow-sm">No hay resultados para su búsqueda.</p>`
    }
    this.hideLoaderIcon()
  ...
  injectHTML() {
    ...
    <div class="search-overlay-top shadow-sm">
      <div class="container container--narrow">
        ...
        <input autocomplete="off" type="text" id="live-search-field" class="live-search-field" placeholder="Buscar...">
